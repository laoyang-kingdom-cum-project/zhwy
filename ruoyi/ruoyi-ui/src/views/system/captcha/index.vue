    ...
    <el-row :gutter="10" class="mb8">
      ...
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:captcha:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:captcha:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:captcha:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="captchaList" @selection-change="handleSelectionChange">
...
<script>
import { listCaptcha, getCaptcha, delCaptcha, addCaptcha, updateCaptcha } from "@/api/system/captcha"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listCaptcha(this.queryParams).then(response => {
        this.captchaList = response.rows
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateCaptcha(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addCaptcha(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/captcha/export', {
        ...this.queryParams
      }, `captcha_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
