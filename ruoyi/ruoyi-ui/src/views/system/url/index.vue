...
<script>
import { listUrl, getUrl, delUrl, addUrl, updateUrl } from "@/api/system/url"
...
export default {
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除图片上传编号为"' + ids + '"的数据项？').then(function() {
        return delUrl(ids)
      }).then(() => {
        this.getList()
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/url/export', {
        ...this.queryParams
      }, `url_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
