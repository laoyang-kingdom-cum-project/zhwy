    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['system:mycaptcha:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['system:mycaptcha:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['system:mycaptcha:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleExport"
          v-hasPermi="['system:mycaptcha:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="mycaptchaList" @selection-change="handleSelectionChange">
...
<script>
import { listMycaptcha, getMycaptcha, delMycaptcha, addMycaptcha, updateMycaptcha } from "@/api/system/mycaptcha"
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true
      listMycaptcha(this.queryParams).then(response => {
        this.mycaptchaList = response.rows
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.id != null) {
            updateMycaptcha(this.form).then(response => {
              this.$modal.msgSuccess("修改成功")
              this.open = false
              this.getList()
            })
          } else {
            addMycaptcha(this.form).then(response => {
    ...
    /** 导出按钮操作 */
    handleExport() {
      this.download('system/mycaptcha/export', {
        ...this.queryParams
      }, `mycaptcha_${new Date().getTime()}.xlsx`)
    }
  }
}
</script>
