  ...
  methods: {
	 ...
	 chooseAvatar() {
      uni.chooseImage({
        ...
        success: async (res) => {
          ...
          try {
            // 直接调用头像更新接口上传文件并修改头像            const avatarRes = await updateAvatar(tempFilePath)
            if (avatarRes.code === 200) {
              // 使用返回的头像路径更新本地显示              const avatarPath = avatarRes.imgUrl || avatarRes.fileName || ''
    ...
    async saveProfile() {
      ...
      try {
        ...
        if (res.code === 200) {
          // 保存住址到本地          uni.setStorageSync('userAddress', this.form.address)
